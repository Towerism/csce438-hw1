 ...
 * */
    ...
    if(command == "CREATE" || command == "DELETE" || command == "JOIN"){
       ...
       //cout << "********    Message sent, awaiting result  *******" << endl;         //cout << "Message: |" << message_to_send << "|" << endl;
       int bytesReceived = 0;
       int rec_result = read(sockfd, buffer, BUFFER_LENGTH);
       string s= string(buffer,strlen(buffer));
       // Check if buffer is a control packet and needs to be processed
...
void chatroom(char *host, int port, string name){
  ...
  while(true){
      ...
      int result  = select(maxfd + 1, &fds, NULL, NULL, NULL);
			// No timeout, no error sockets, no write sockets
      if (result == -1){
			// Some error occurred!
